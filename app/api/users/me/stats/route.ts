...
import { getProfileStatsForIdentity } from "@/modules/users/profile-stats";
...
export async function GET(request: Request) {
  ...
  const rl = rateLimitSync(`users:me:stats:${ip}`, 120);
  if (!rl.ok) {
  ...
  try {
    const stats = await getProfileStatsForIdentity(userId, parsed.data.identityId);
    return withApiSecurityHeaders(NextResponse.json(stats));
  } catch (e) {
