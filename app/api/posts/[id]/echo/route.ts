...
import { toggleEcho } from "@/modules/post-interactions/service";
...
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const rl = rateLimitSync(`posts:echo:${userId}:${ip}`, 120);
  if (!rl.ok) {
  ...
  try {
    const out = await toggleEcho(userId, postId, parsed.data.identityId);
